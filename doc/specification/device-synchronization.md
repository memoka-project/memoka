...
IME変換中は開始前の確定済み投影と最新のEditor文書を保持し、途中の文字変更をYjsへ書き込まない。
`compositionend`後のDOM入力を取り込んでから、最終差分だけを一つのYjs transactionで記録する。
変換開始時のUndo単位を保持し、確定直後にEscでInsertを終了しても確定文字を同じ変更単位へ含める。
取消で元へ戻った場合は更新を作らない。未確定文字は永続文書、同期journal、別Window、バックアップへ含めない。
保存・CLI読出しは既に終了した変換の反映を完了させるが、進行中のIMEを強制確定しない。
通常終了・Note切替・Workspace切替はフォーカス移動による確定・取消を処理し、終了しない未確定入力は破棄する。
未確定入力の復旧用保存は追加しない。既存のYjs履歴を書き換えず、変更後の入力に適用する。
...
通常配送はWorkspace単位の単調時計でまとめる。ローカル変更を観測してから500 msの無入力、または最初の
待機変更から2秒で、その時点までの署名済み変更の配送を許可する。保存と署名自体は遅延しない。
再起動・再接続、`:sync`、初回複製・checkpoint配送は待機を解除し、他端末由来の変更には入力待ちを追加しない。
バッチの連番・署名・依存関係・通信形式は維持し、既存バッチを再採番・書換えしない。
...
GUIの受信通知は最初の通知から固定100 msで集約し、後の通知で期限を延ばさない。
反映は最大128 batch・合計4 MiBを一組とし、依存を満たす順で候補文書を検証する。
4 MiBを超える正当な単独batchは従来の上限内で単独処理する。文書ごとの差分を統合して、
全文書、添付metadata、各batchのreceipt、frontierを一つのowner transactionで保存する。
GUIへは文書ごとに一つのupdateを渡し、反映・索引更新予約をまとめる。続きがある間は描画へ制御を戻して
即時に次の組を処理し、各batchへの250 ms待機は行わない。IME・競合等による保留は250 msで再試行する。
checkpointで組の一部が既に反映された場合は残りを再準備し、破損は原因batchだけを隔離する。
...
接続は同時8件まで、連続した接続失敗には2・4・8・16・30秒の間隔で再試行する。
認証済みの接続を確認すると過去の失敗回数をリセットする。90秒ごとの接続更新は失敗回数へ加算せず、
1秒後に再接続する。`:sync`は再試行待ちを解除する。通常の変更配送で30秒を待つことはない。
並行した応答が完了しても、別streamで読み取り中のリクエストheaderを中断しない。
接続失敗時は端末の公開鍵の短い識別情報、error code、次回の再試行待ち時間をログへ記録する。
