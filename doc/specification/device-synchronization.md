...
`:sync-settings`の通常画面は「このWorkspaceの同期状態」「登録端末」「端末を追加」を中心とする。
待受設定、公開鍵、隔離データ、添付の進捗・再試行、現在Noteの保護内容の復旧への入口は詳細に置く。
一時停止・再開・登録解除・鍵を照合した接続先アドレス更新も通常画面の詳細から行う。
承認待ちは通常画面の目立つ位置に表示し、待機件数は「この端末で反映待ち」「相手への送信待ち」
「相手で未反映（未受信分を含む）」の方向を区別する。未接続端末を同期済みと表示しない。
