...
`:sync-settings`は「状態」「他端末」の2画面に分ける。紹介文は設定画面に置かない。
状態画面は同期の操作（一時停止・再開・今すぐ同期）と自端末の状態（公開鍵・待受状態・待ち受けアドレス変更）、
受信（最終反映・反映待ち・添付取得待ち・検証失敗の詳細）、送信待ちを表示する。
待機件数は「反映待ち」「送信待ち」「相手で未反映（未受信分を含む）」の方向を区別し、
未接続端末を同期済みと表示しない。他端末画面は端末追加・参加承認待ちと、
公開鍵・接続状態・アドレス更新・反映待ち・最終反映・登録解除を行う他端末ごとの一覧を表示する。
待受アドレス変更・接続先アドレス更新・登録解除確認は専用の副画面で行い、Escやキャンセルで元の画面へ戻る。
既存の絶対日時・ago表示を使う。デバッグラインにも接続数・反映待ち・添付取得待ちを表示する。
この端末自身の登録を解除した場合、認可記録の作成後に同期状態を消去し、Workspaceを同期未設定へ戻す。
他端末から解除された場合は状態タブへ「登録解除済み」を表示し、「同期を未設定に戻す」操作で同じ消去を行う。
消去は同期テーブルとOS資格情報の対象鍵のみとし、文書・履歴・添付・バックアップは変更しない。

「端末を追加」は同じmodal内の専用表示へ切り替え、「元端末で準備」「元端末で招待」「新しい端末で受信」
「元端末で承認」の番号付き手順と各段階の操作端末を表示する。招待用の文字列は「招待コード」、
登録後のIP・port変更は「接続先アドレス」に用語を統一する。
アドレス候補は追加画面を開いた時と「候補を更新」で`sync_address_candidates(workspaceId)`から取得する。
OSのinterface一覧と実際の待受portを組み合わせ、loopback・未指定・multicast・IPv6 link-localを除外し、
未選択で提示する。候補がなくても手入力で進められる。候補取得は外部へ探索・送信しない。
招待作成結果は`invitationId`と`expiresAt`を含み、招待コードはWorkspaceを開いている間だけメモリに保持して
設定を開き直しても再コピーできる。期限切れ・承認完了・Workspace切替で消去し、ファイルには保存しない。
参加端末は`:new-workspace`、起動時のWorkspace選択画面、または同期設定の入口から「別端末から受信」を開く。
「初めて受信」と「中断した受信を再開」を別操作とし、再開は保存先だけで進められる。
Workspaceを開いている場合も別保存先で受信し、本文受信後に旧Workspaceの保存とバックアップを待って切り替える。
元端末で候補の名前と鍵の識別情報を確認して承認した後にだけ文書を受け取る。
参加用の秘密情報はOS資格情報へ置き、DBには公開の再開記録だけを残す。
