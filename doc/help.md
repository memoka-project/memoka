...
**戻る・Esc・Ctrl-C**で詳細/追加画面から一覧へ戻り、一覧からは画面を閉じます。未保存の入力がある場合は破棄を確認します。
...
3. 接続、保持数、**バックアップ用パスワード**を指定して「保存先を登録」を押します。パスワードは同じものを2回入力します。成功後は一覧へ戻ります。
...
Driveでは新しい世代の**転送を優先**し、転送先の内容検証と保持整理は後のアイドル時に実行します。
...
Driveの専用フォルダー名は`MemokaBackup-<Workspace ID>-<保存先ID>`です。
