...
終了時は**時間制限を設けず転送完了を待ちます**。30秒の待機タイムアウトはありません。
同じ起動中に確認済みのバックアップがあり、その後の編集も未転送もなければ、全ノートの再検証や履歴一覧の再取得、新しい転送処理の起動を省略します。
新しい編集、保存先の追加・変更、ローカル履歴ファイルの変更や転送失敗があれば、必要なバックアップ確認を行います。再起動後の初回確認は省略しません。
Driveの未開始の検証・保持整理は終了を待たせず、次回起動後に再開します。すでに実行中の検証・整理は安全に処理を終えるまで待ちます。
これは転送先の完全な検証を待つ終了ではありません。「保護済み」を確認したい場合は`:backup-settings`で検証完了も確認してください。
通信自体の接続・無応答タイムアウトと各処理単位の1時間上限は残ります。実際の転送失敗は表示し、自動で無視して終了しません。
長く待ちたくない場合は、終了操作を取り消すか、バックアップを中断して続行するかを選んでください。
**「終了を取り消す」**（`Esc` / `Ctrl-c`も同じ）は終了待ちだけを解除し、編集中の画面に戻ります。
`:backup`や自動実行で始まった転送・検証はバックグラウンドで続きます。転送中止や失敗扱いにはしません。
**「バックアップを中断して終了」**は転送自体を止め、処理の終了を待ってからアプリを閉じます。
終了を取り消してから、改めて`:qa`で転送の完了を待つこともできます。
...
切替・更新でも「取り消す」はバックアップを継続します。「バックアップを中断して続行」では実行中のバックアップ処理の終了を待ってから切替・更新へ進みます。
