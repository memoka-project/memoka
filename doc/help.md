...
IME変換途中の文字は入力中のEditor内だけに保持し、確定した内容から自動保存・同期します。
変換途中の文字は、別Windowや別端末には表示せず、異常終了後の復旧対象にも含めません。
終了やノート・Workspace切替では通常のフォーカス移動による確定・取消を処理します。
それでも未確定のまま残った入力は破棄し、確定済みの内容を保存して操作を続けます。
...
開閉状態はWindowごとに保存し、ノートを開き直した場合やアプリ再起動後も復元します。別Windowには影響しません。
...
| `0` / `$`                           | 論理行の先頭、末尾へ移動します。                          |
| `[count]f{文字}` / `[count]F{文字}` | 論理行内で前/後の指定文字へ移動します。                   |
| `[count];` / `[count],`             | 直前の文字検索を同方向/逆方向へ繰り返します。             |
...
| `[count]n` / `[count]N`             | 直前のノート内検索と同方向、逆方向の一致へ移動します。    |

`whichwrap = true`では、`h/l/w/b/e/W/B/E`が論理行端を越えて前後の論理行へ移動します。
`false`では論理行端で停止します。
`f/F/;/,`は論理行を越えません。Tableでは同じrow内のCellをまたいで検索できます。`f/F`のあとに半角英数記号を入力するとVimと同じ文字検索になります。
Visual Charでも`f/F/;/,`を使えます。選択の起点を保ったままheadを移動し、phrase hintも使えます。
Visual Charの`t/T{文字}`は指定文字の直前/直後までheadを移動します。Normal単独の`t/T`は文字検索に使わず、`t`の既存Tab操作を維持します。
Normalの`d/y/c`では`f/F/t/T{文字}`をmotionに指定できます。`f/F`は指定文字を含み、`t/T`は指定文字を含めません。
Countなしの`f/F/t/T`では日本語を含むphraseの先頭にも英字1〜3文字のhintを表示します。小文字の1〜3文字labelを使い切った場合だけ大文字も使います。hintは設定中の日本語word分割に従い、
半角英数字・半角記号で始まるphraseも対象です。2文字以上のhintは入力途中で候補を絞り込みます。
`t/T`のhint確定時はphrase先頭文字の直前/直後へ移動します。Count付きの`f/F/t/T`はhintを表示しません。`Esc`でhint入力を取り消せます。
hint移動後の`;`/`,`は選んだphraseの先頭文字を探し、同じ文字がphraseの途中にあっても移動します。
...
| `[[` / `]]`                 | 前、次の表示中Sectionのタイトルへ移動します。       |
| `{` / `}`                   | 前、次の表示中blockの先頭へ移動します。             |
...
折り畳み状態はWindowごとの表示状態としてアプリ再起動後も復元しますが、本文、Undo履歴、別Windowには反映しません。
表示中の行をクリックしても折り畳みを維持します。検索や位置復元などで隠れた行へ移動すると自動的に展開します。64 KiBを超えるコードと
未対応の言語はhighlightしませんが、編集、折り畳み、copyは利用できます。これらの機能はSource Blockには適用しません。
...
箇条書きの前後に箇条書きを作成・貼り付けすると、項目は1つの箇条書きへまとまります。
番号付きリスト同士も同様で、左側の開始番号を保ちます。箇条書きと番号付きリストは別のままです。
タスクは箇条書き項目の状態なので、通常項目・未完了・完了を同じ箇条書き内に混在できます。
この統合はSection本文、リスト項目内、Details本文で共通で、作成・貼り付けと一緒に`u`で戻せます。
...
Tree、`<Leader>f`のノート名検索、`<Leader>b`のBuffer検索から以前開いたNoteへ戻ると、Windowごとに記憶したカーソル位置・Focused Section・スクロール位置を復元します。復元直後の再描画で高さが変わっても、カーソルの画面内位置を基準にスクロールを補正します。再起動後も保持します。初めて開くNoteはタイトル先頭から始まります。Section名や本文の検索結果など、移動先を指定した操作ではその移動先を優先します。
...
グループも検索できます。`r`またはプレビュー下の「復元 (r)」で同じ削除操作の対象を復元します。`Shift-d`（`D`）または「Trashから削除 (Shift-d)」は確認後、その操作の対象をTrashから取り除き、通常の操作では復元できなくします。管理HelpはTrashから削除できません。本文データは物理的に消去されず、現在の保存データや過去のバックアップに残る場合があります。`Enter`と`Tab`では閉じず、`Esc`または`Ctrl-c`で終了します。
...
Normalの`[count]*`はカーソル上の語を前方、`[count]#`は後方に検索します。漢字・ひらがな・カタカナ・英数字などを文字種別ごとの語として扱い、日本語word分割設定には左右されません。語の上にいなければ同じ論理行の右側の語を探し、語がなければ記号の連続部分を探します。NFKC正規化と大文字小文字を無視した語全体一致で、Migemoやあいまい検索は使いません。検索範囲は`/`/`?`と同じで、以後の`n/N`は最後の`*`または`#`の方向を基準に巡回します。該当する語がないときは前回の検索を保持します。

`/`または`?`で小文字のローマ字（途中の`-`も可。例: `no-to`→「ノート」）を入力すると、画面内のword候補に英語のあいまい一致とMigemoによる日本語検索を行い、
一致箇所に大文字1〜3文字のジャンプhintを表示します。表示された大文字labelを入力するとその位置へ移動し、
以後の`n/N`はlabelを除いた小文字の検索語で、Focused Section内の候補を最後の検索方向に従って巡回します。
1〜128文字で先頭が小文字、残りが小文字か`-`の検索語で`Enter`を押すと、hintを選ばずにFocused Section全体をMigemo＋あいまい検索し、
指定方向の次の一致へ移動します。画面内に候補がなくても検索でき、その後の`n/N`で候補を巡回できます。
先頭が大文字、hintに該当しない大文字、日本語・数字・`-`以外の記号（検索語中の`?`を含む）を含む入力、またはhint入力中の`Enter`は、
入力欄の文字列をそのまま使う従来の部分一致検索です。Migemo辞書を読み込めない場合も従来検索を使います。
一致しない場合もエラーを表示してNormalに戻ります。
...
候補は各検索で最大100件を表示します。
入力中の`Ctrl-h`は直前の文字（選択範囲があればその範囲）、`Ctrl-u`は行頭からカーソル直前までを削除します。
`f`はNoteを1件ずつ表示し、Section titleは検索しません。Note名を先に、`/`の左右に空白を入れた祖先pathを後に表示します。
Note候補にはファイルアイコンを表示します。Note名の右の緑の●は現在のWindow、青の●は別のWindowで開いていることを示します。
`f`の結果を開くと、既に同じNoteを表示している場合もそのEditorへfocusを移します。
`s`は一致した本文行ごとに表示します。結果を開くとSectionにfocusせず、Note全体を表示して一致箇所へ移動し、`zz`と同様にカーソルをWindowの上下中央へ配置します。小文字ローマ字（`no-to`、`kensaku`など）はMigemoで日本語にも一致するため、IMEをOFFにしたまま検索できます。
...
既定LeaderはSpaceです。`config.toml`では物理Leader keyだけを変更でき、その後に続くcategory keyは固定です。
既存設定で`f`、`F`、`;`、`,`を指定した場合は、設定fileを変更せずSpaceとして読み込み、警告を表示します。
...
| `:new-workspace`                                  | 空のWorkspaceを作成するか、別端末から受信します。                                  |
...
| `:sync-settings`                                  | 同期状態、端末一覧、追加、接続先アドレス更新、一時停止・再開、登録解除を開きます。 |
...
1. 元端末で`:sync-settings`を開き、「端末を追加」の案内に従って端末名を入力し、同期を有効にします。待受設定は既定値を使い、変更は詳細から行います。
2. 元端末の案内で、この端末へ到達できるLAN・VPNアドレスを候補から選ぶか手入力します。候補には実際の待受UDP portが組み合わされます。
3. 「招待コードを作成」で作成した招待コードをコピーボタンで参加端末へ渡します。有効期限は10分で、一回限り使えます。
4. 新しい端末で`:new-workspace`を開き、「別端末から受信」を選びます。初回起動時はWorkspace選択画面から、同期設定からも開けます。初めての受信では招待コード・端末名・新しい空の保存先を指定します。
5. 元端末の同期設定に現れる名前と鍵の識別情報を参加端末の表示と照合し、承認します。
6. 参加端末で文書の受信後にWorkspaceを開きます。現在開いているWorkspaceがある場合は、その保存とバックアップの完了を待って切り替えます。添付ファイルは引き続き取得されます。

既存のWorkspaceコピーやバックアップ復旧先へ合流することはできません。初回受信を中断した場合は、
「別端末から受信」の「中断した受信を再開」で同じ保存先だけを選んで再開できます。保存した登録情報を使うため、招待コードと端末名の再入力は不要です。
ネットワークが変わった場合は、同期設定の端末詳細から同じ公開鍵の接続先アドレスを更新します。
...
同期設定は「状態」「他端末」の2画面に分かれます。状態画面では同期の操作（一時停止・再開・今すぐ同期）と、
自端末の状態（公開鍵・待受状態・待ち受けアドレス変更）、受信（最終反映・反映待ち・添付取得待ち・検証失敗）、
送信待ちを確認します。他端末画面では参加している他端末を一覧表示し、公開鍵・接続状態・アドレス更新・
反映待ち・最終反映・登録解除を端末ごとに確認します。参加承認待ちも他端末画面に表示します。
待受アドレスの変更と接続先アドレスの更新、登録解除の確認はそれぞれ専用画面で行い、
Escやキャンセルで元の画面に戻ります。待機件数は「反映待ち」「送信待ち」
「相手で未反映（未受信分を含む）」の方向を区別して表示し、「接続済み」だけでは反映完了を意味しません。
未接続端末は、待機件数が0でも「未接続」と表示します。
有効な招待コードはWorkspaceを開いている間だけメモリに保持し、同期設定を開き直しても再コピーできます。
期限切れ・承認完了・Workspace切替で消去し、ファイルには保存しません。
取得中の添付は本文に「別端末から取得待ち」と表示し、失敗した添付は同期詳細から再試行できます。
通常は自動で再接続します。今すぐ確認する場合は`:sync`を使います。

確定済みの編集はローカルへ順次保存し、入力が約500 ms止まった時点でまとめて送ります。
入力が続いても、送信待ち開始から約2秒ごとに配送します。再接続時の蓄積分と`:sync`は待機を解除します。
受信した差分はまとめて本文へ反映します。受信側で対象ノートをIME変換中の場合は、確定・取消まで反映を待ちます。

どの登録端末からも別端末を追加・解除できます。解除した鍵は再利用できず、参加し直す場合は新しい保存先へ複製します。
解除前に相手へ渡したデータは消去できません。一時的に通信を止めるだけなら一時停止を使います。
この端末自身の登録を解除すると、Workspaceは同期未設定の状態に戻ります。他端末から解除された場合は、
状態タブに「登録解除済み」と表示され、「同期を未設定に戻す」で同じ状態に戻せます。戻した後は新しい
同期グループの元端末として使い直せますが、旧グループへ再合流することはできません。
...
新しいWorkspaceは`:new-workspace`から「空のWorkspaceを作成」または「別端末から受信」を選びます。
作成には空の保存先が必要です。受信や切り替えを取り消した場合は現在のWorkspaceへ戻れます。
