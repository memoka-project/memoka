        ...
        table(
          ...
          [
            ...
            [
              ...
              "文字または論理行・構造をClipboardへコピー。画像1 blockだけなら画像dataも同時に公開",
            ],
            [
              "Paste",
              rich(code("p / P")),
              "カーソルの後・前へregister、または外部でcopyしたfile・画像dataを貼り付け",
            ],
            [
              "履歴",
              rich(code("u / Ctrl-r / .")),
            ...
            [
              "画像をWindowで開く",
              rich(code("gf / Ctrl-w gf")),
              "画像を現在Window／新しいTabPageで開く。画像WindowのCtrl-oで元位置へ戻る",
            ],
            [
              "Section focus",
              rich(code("zf / zF")),
        ...
        bulletList("notes", [
          ...
          listItem(
            ...
            rich(
              "Attachment Fileまたは",
              code(":attach"),
              "でファイルを追加できます。PNG・JPEG・WebP・GIFは画像、それ以外は1つのAttachment Blockとして扱います。file paste / dropに加え、OS Clipboard上の画像dataをInsert modeのpasteまたはNormalの",
              code("p / P"),
              "でPNGとして取り込めます。Count付きputでも画像dataのCAS取り込みは1回だけです。clickは選択だけ、",
              code("gx"),
              "で安全な形式だけをOS既定アプリへ渡します。",
            ),
          ),
          listItem(
            "images",
            rich(
              ...
              "またはVisual Lineの",
              code("y"),
              "でyankすると、Memoka構造・Markdown・file形式と同時にOSの画像dataとしてコピーします。画像と他blockの混在yankでは画像dataを公開しません。Normal modeで画像を選択またはhoverすると右下にresize handleが現れ、drag中は見た目だけ、pointerを離した時に1 Undo単位で確定します。",
              code(":image-width"),
              "は現在幅を表示し、",
              code(":image-width 50"),
              "または",
              code(":image-width 50%"),
              "でノート表示幅の10〜100%へ変更します。100%は通常のMarkdown画像、狭めた画像は幅を保つ限定的な",
              code('<img width="50%">'),
              "としてClipboardとmirrorを往復します。画像上の",
              code("gf"),
              "は現在Window、",
              code("Ctrl-w gf"),
              "は新しいTabPageへ画像Bufferを開きます。画像は拡大せず収まる大きさで中央表示され、",
              code("Ctrl-o"),
              "で同じsession中の元位置へ戻ります。画像Bufferは再起動後も復元され、",
              code(":buffers"),
              "では📷付きで選べます。",
            ),
          ),
